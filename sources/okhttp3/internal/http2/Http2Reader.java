...
final class Http2Reader implements Closeable {
    ...
    private void readGoAway(Handler handler, int i, byte b, int i2) throws IOException {
        if (i < 8) {
            ...
            if (fromHttp2 != null) {
                ...
                if (i3 > 0) {
                    byteString = this.source.b0((long) i3);
                }
            ...
            if (!z || readByte == 4) {
                ...
                switch (readByte) {
                    ...
                    case 8:
                        ...
                        break;
                    default:
                        this.source.d0((long) readMedium);
                        break;
                }
                return true;
            }
    ...
    public void readConnectionPreface(Handler handler) throws IOException {
        if (!this.client) {
            ...
            ByteString b0 = bufferedSource.b0((long) byteString.A());
            Logger logger2 = logger;
            if (logger2.isLoggable(Level.FINE)) {
                logger2.fine(Util.format("<< CONNECTION %s", b0.o()));
            }
            if (!byteString.equals(b0)) {
                throw Http2.ioException("Expected a connection header but was %s", b0.E());
            }
    ...
    public static final class ContinuationSource implements Source {
        ...
        public long read(Buffer buffer, long j) throws IOException {
            while (true) {
                int i = this.left;
                if (i == 0) {
                    this.source.d0((long) this.padding);
                    this.padding = 0;
                    if ((this.flags & 4) != 0) {
                        return -1;
                    }
