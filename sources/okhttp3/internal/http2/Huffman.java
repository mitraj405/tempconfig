...
class Huffman {
    ...
    private void addCode(int i, int i2, byte b) {
        ...
        while (b > 8) {
            b = (byte) (b - 8);
            int i3 = (i2 >>> b) & Constants.MAX_HOST_LENGTH;
    ...
    public byte[] decode(byte[] bArr) {
        ...
        for (byte b2 : bArr) {
            ...
            i += 8;
            while (i >= 8) {
                node = node.children[(b >>> (i - 8)) & Constants.MAX_HOST_LENGTH];
        ...
        while (i > 0) {
            Node node2 = node.children[(b << (8 - i)) & Constants.MAX_HOST_LENGTH];
    ...
    public int encodedLength(ByteString byteString) {
        ...
        for (int i = 0; i < byteString.A(); i++) {
            j += (long) CODE_LENGTHS[byteString.n(i) & UnsignedBytes.MAX_VALUE];
        }
        return (int) ((j + 7) >> 3);
    }
