package defpackage;

import android.view.View;
import cris.org.in.ima.fragment.MakePaymentNewFragment;

/* renamed from: Yj  reason: default package and case insensitive filesystem */
/* compiled from: MakePaymentNewFragment */
public final class C0759Yj implements View.OnClickListener {
    public final /* synthetic */ MakePaymentNewFragment a;

    public C0759Yj(MakePaymentNewFragment makePaymentNewFragment) {
        this.a = makePaymentNewFragment;
    }

    public final void onClick(View view) {
        MakePaymentNewFragment makePaymentNewFragment = this.a;
        makePaymentNewFragment.c = 7;
        makePaymentNewFragment.g(91);
    }
}
