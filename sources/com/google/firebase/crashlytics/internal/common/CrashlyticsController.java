...
class CrashlyticsController {
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        ...
        if (i >= 30) {
            List l = ((ActivityManager) this.context.getSystemService("activity")).getHistoricalProcessExitReasons((String) null, 0, 0);
            if (l.size() != 0) {
                this.reportingCoordinator.persistRelevantAppExitInfoEvent(str, l, new LogFileManager(this.fileStore, str), UserMetadata.loadFromExistingSession(str, this.fileStore, this.backgroundWorker));
                return;
            }
            Logger logger = Logger.getLogger();
