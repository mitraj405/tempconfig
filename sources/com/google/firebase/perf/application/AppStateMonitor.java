...
public class AppStateMonitor implements Application.ActivityLifecycleCallbacks {
    ...
    private void startFrameMonitoring(Activity activity) {
        if (isScreenTraceSupported() && this.configResolver.isPerformanceMonitoringEnabled()) {
            ...
            if (activity instanceof FragmentActivity) {
                ...
                ((FragmentActivity) activity).getSupportFragmentManager().f2079a.f2153a.add(new n.a(fragmentStateMonitor));
            }
        }
    }
    ...
    public void onActivityDestroyed(Activity activity) {
        ...
        if (this.activityToFragmentStateMonitorMap.containsKey(activity)) {
            ...
            FragmentManager.l remove = this.activityToFragmentStateMonitorMap.remove(activity);
            n nVar = supportFragmentManager.f2079a;
            synchronized (nVar.f2153a) {
                int size = nVar.f2153a.size();
                int i = 0;
                while (true) {
                    if (i >= size) {
                        break;
                    } else if (nVar.f2153a.get(i).a == remove) {
                        nVar.f2153a.remove(i);
                        break;
                    } else {
                        i++;
                    }
