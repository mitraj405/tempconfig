...
final class Arrangement {
    ...
    public Arrangement(int i, float f, float f2, float f3, int i2, float f4, int i3, float f5, int i4, float f6) {
        this.priority = i;
        this.smallSize = C1354qp.w(f, f2, f3);
        this.smallCount = i2;
        this.mediumSize = f4;
        this.mediumCount = i3;
        this.largeSize = f5;
    ...
    private void fit(float f, float f2, float f3, float f4) {
        ...
        if (i > 0 && space > 0.0f) {
            ...
            this.smallSize = Math.min(space / ((float) i), f3 - f6) + f6;
        } else if (i > 0 && space < 0.0f) {
            float f7 = this.smallSize;
            this.smallSize = Math.max(space / ((float) i), f2 - f7) + f7;
        }
        ...
        if (i3 > 0 && calculateLargeSize != f4) {
            ...
            float min = Math.min(Math.abs(f9), f8 * MEDIUM_ITEM_FLEX_PERCENTAGE * ((float) i3));
            if (f9 > 0.0f) {
