...
public abstract class zzji<T extends zzix> {
    ...
    public static <T extends zzix> T zza(Class<T> cls) {
        ...
        try {
            return (zzix) cls.cast(((zzji) Class.forName(str, true, classLoader).getConstructor(new Class[0]).newInstance(new Object[0])).zza());
            ...
            while (it.hasNext()) {
                try {
                    ...
                    Logger.getLogger(zzit.class.getName()).logp(Level.SEVERE, "com.google.protobuf.GeneratedExtensionRegistryLoader", "load", "Unable to load ".concat(cls.getSimpleName()), e5);
                }
            }
            if (arrayList.size() == 1) {
