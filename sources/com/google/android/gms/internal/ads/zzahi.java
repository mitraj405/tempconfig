package com.google.android.gms.internal.ads;

import android.util.Pair;
import com.google.common.primitives.UnsignedBytes;
import com.google.firebase.perf.util.Constants;
import com.google.logging.type.LogSeverity;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.Collections;
import java.util.List;

/* compiled from: com.google.android.gms:play-services-ads@@23.3.0 */
public final class zzahi {
    public int zzA = -1;
    public int zzB = 1000;
    public int zzC = LogSeverity.INFO_VALUE;
    public float zzD = -1.0f;
    public float zzE = -1.0f;
    public float zzF = -1.0f;
    public float zzG = -1.0f;
    public float zzH = -1.0f;
    public float zzI = -1.0f;
    public float zzJ = -1.0f;
    public float zzK = -1.0f;
    public float zzL = -1.0f;
    public float zzM = -1.0f;
    public byte[] zzN;
    public int zzO = 1;
    public int zzP = -1;
    public int zzQ = 8000;
    public long zzR = 0;
    public long zzS = 0;
    public zzadq zzT;
    public boolean zzU;
    public boolean zzV = true;
    public zzadp zzW;
    public int zzX;
    /* access modifiers changed from: private */
    public int zzY;
    /* access modifiers changed from: private */
    public String zzZ = "eng";
    public String zza;
    public String zzb;
    public int zzc;
    public int zzd;
    public int zze;
    public int zzf;
    public boolean zzg;
    public byte[] zzh;
    public zzado zzi;
    public byte[] zzj;
    public zzy zzk;
    public int zzl = -1;
    public int zzm = -1;
    public int zzn = -1;
    public int zzo = -1;
    public int zzp = -1;
    public int zzq = 0;
    public int zzr = -1;
    public float zzs = 0.0f;
    public float zzt = 0.0f;
    public float zzu = 0.0f;
    public byte[] zzv = null;
    public int zzw = -1;
    public boolean zzx = false;
    public int zzy = -1;
    public int zzz = -1;

    private static Pair zzf(zzek zzek) throws zzbo {
        try {
            zzek.zzL(16);
            long zzs2 = zzek.zzs();
            if (zzs2 == 1482049860) {
                return new Pair("video/divx", (Object) null);
            }
            if (zzs2 == 859189832) {
                return new Pair("video/3gpp", (Object) null);
            }
            if (zzs2 == 826496599) {
                int zzd2 = zzek.zzd() + 20;
                byte[] zzM2 = zzek.zzM();
                while (true) {
                    int length = zzM2.length;
                    if (zzd2 < length - 4) {
                        int i = zzd2 + 1;
                        if (zzM2[zzd2] == 0 && zzM2[i] == 0 && zzM2[zzd2 + 2] == 1 && zzM2[zzd2 + 3] == 15) {
                            return new Pair("video/wvc1", Collections.singletonList(Arrays.copyOfRange(zzM2, zzd2, length)));
                        }
                        zzd2 = i;
                    } else {
                        throw zzbo.zza("Failed to find FourCC VC1 initialization data", (Throwable) null);
                    }
                }
            } else {
                zzea.zzf("MatroskaExtractor", "Unknown FourCC. Setting mimeType to video/x-unknown");
                return new Pair("video/x-unknown", (Object) null);
            }
        } catch (ArrayIndexOutOfBoundsException unused) {
            throw zzbo.zza("Error parsing FourCC private data", (Throwable) null);
        }
    }

    private static List zzg(byte[] bArr) throws zzbo {
        byte b;
        byte b2;
        try {
            if (bArr[0] == 2) {
                int i = 0;
                int i2 = 1;
                while (true) {
                    byte b3 = bArr[i2];
                    i2++;
                    b = b3 & UnsignedBytes.MAX_VALUE;
                    if (b != 255) {
                        break;
                    }
                    i += Constants.MAX_HOST_LENGTH;
                }
                int i3 = i + b;
                int i4 = 0;
                while (true) {
                    byte b4 = bArr[i2];
                    i2++;
                    b2 = b4 & UnsignedBytes.MAX_VALUE;
                    if (b2 != 255) {
                        break;
                    }
                    i4 += Constants.MAX_HOST_LENGTH;
                }
                int i5 = i4 + b2;
                if (bArr[i2] == 1) {
                    byte[] bArr2 = new byte[i3];
                    System.arraycopy(bArr, i2, bArr2, 0, i3);
                    int i6 = i2 + i3;
                    if (bArr[i6] == 3) {
                        int i7 = i6 + i5;
                        if (bArr[i7] == 5) {
                            int length = bArr.length - i7;
                            byte[] bArr3 = new byte[length];
                            System.arraycopy(bArr, i7, bArr3, 0, length);
                            ArrayList arrayList = new ArrayList(2);
                            arrayList.add(bArr2);
                            arrayList.add(bArr3);
                            return arrayList;
                        }
                        throw zzbo.zza("Error parsing vorbis codec private", (Throwable) null);
                    }
                    throw zzbo.zza("Error parsing vorbis codec private", (Throwable) null);
                }
                throw zzbo.zza("Error parsing vorbis codec private", (Throwable) null);
            }
            throw zzbo.zza("Error parsing vorbis codec private", (Throwable) null);
        } catch (ArrayIndexOutOfBoundsException unused) {
            throw zzbo.zza("Error parsing vorbis codec private", (Throwable) null);
        }
    }

    private static boolean zzh(zzek zzek) throws zzbo {
        try {
            int zzk2 = zzek.zzk();
            if (zzk2 == 1) {
                return true;
            }
            if (zzk2 == 65534) {
                zzek.zzK(24);
                if (zzek.zzt() == zzahj.zze.getMostSignificantBits() && zzek.zzt() == zzahj.zze.getLeastSignificantBits()) {
                    return true;
                }
                return false;
            }
            return false;
        } catch (ArrayIndexOutOfBoundsException unused) {
            throw zzbo.zza("Error parsing MS/ACM codec private", (Throwable) null);
        }
    }

    private final byte[] zzi(String str) throws zzbo {
        byte[] bArr = this.zzj;
        if (bArr != null) {
            return bArr;
        }
        throw zzbo.zza("Missing CodecPrivate for codec ".concat(String.valueOf(str)), (Throwable) null);
    }

    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r2v18, resolved type: java.lang.Object} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r17v16, resolved type: java.lang.String} */
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r1v66, resolved type: java.util.ArrayList} */
    /* JADX WARNING: Can't fix incorrect switch cases order */
    /* JADX WARNING: Code restructure failed: missing block: B:128:0x024f, code lost:
        r1 = -1;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:135:0x0287, code lost:
        r1 = -1;
        r10 = -1;
        r17 = "audio/x-unknown";
     */
    /* JADX WARNING: Code restructure failed: missing block: B:144:0x02b9, code lost:
        r10 = -1;
        r1 = 4096;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:148:0x0329, code lost:
        r2 = r1;
        r10 = -1;
        r1 = r16;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:151:0x034c, code lost:
        r2 = r1;
        r1 = -1;
        r10 = -1;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:154:0x0383, code lost:
        r3 = r1;
        r1 = -1;
        r10 = -1;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:164:0x039f, code lost:
        r1 = -1;
        r10 = -1;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:165:0x03a1, code lost:
        r2 = null;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:166:0x03a2, code lost:
        r3 = null;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:168:0x03a5, code lost:
        if (r0.zzN == null) goto L_0x03b8;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:169:0x03a7, code lost:
        r4 = com.google.android.gms.internal.ads.zzace.zza(new com.google.android.gms.internal.ads.zzek(r0.zzN));
     */
    /* JADX WARNING: Code restructure failed: missing block: B:170:0x03b2, code lost:
        if (r4 == null) goto L_0x03b8;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:171:0x03b4, code lost:
        r3 = r4.zza;
        r17 = "video/dolby-vision";
     */
    /* JADX WARNING: Code restructure failed: missing block: B:172:0x03b8, code lost:
        r4 = r17;
        r5 = r0.zzV;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:173:0x03be, code lost:
        if (true == r0.zzU) goto L_0x03c2;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:174:0x03c0, code lost:
        r7 = 0;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:175:0x03c2, code lost:
        r7 = 2;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:176:0x03c3, code lost:
        r5 = r5 | r7;
        r7 = new com.google.android.gms.internal.ads.zzad();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:177:0x03cd, code lost:
        if (com.google.android.gms.internal.ads.zzbn.zzg(r4) == false) goto L_0x03de;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:178:0x03cf, code lost:
        r7.zzy(r0.zzO);
        r7.zzY(r0.zzQ);
        r7.zzR(r10);
     */
    /* JADX WARNING: Code restructure failed: missing block: B:180:0x03e2, code lost:
        if (com.google.android.gms.internal.ads.zzbn.zzi(r4) == false) goto L_0x0582;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:182:0x03e6, code lost:
        if (r0.zzq != 0) goto L_0x03f8;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:183:0x03e8, code lost:
        r6 = r0.zzo;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:184:0x03ea, code lost:
        if (r6 != -1) goto L_0x03ee;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:185:0x03ec, code lost:
        r6 = r0.zzl;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:186:0x03ee, code lost:
        r0.zzo = r6;
        r6 = r0.zzp;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:187:0x03f2, code lost:
        if (r6 != -1) goto L_0x03f6;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:188:0x03f4, code lost:
        r6 = r0.zzm;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:189:0x03f6, code lost:
        r0.zzp = r6;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:190:0x03f8, code lost:
        r6 = r0.zzo;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:191:0x03fc, code lost:
        if (r6 == -1) goto L_0x040c;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:192:0x03fe, code lost:
        r9 = r0.zzp;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:193:0x0400, code lost:
        if (r9 == -1) goto L_0x040c;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:194:0x0402, code lost:
        r9 = ((float) (r0.zzm * r6)) / ((float) (r0.zzl * r9));
     */
    /* JADX WARNING: Code restructure failed: missing block: B:195:0x040c, code lost:
        r9 = -1.0f;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:197:0x040f, code lost:
        if (r0.zzx == false) goto L_0x04f1;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:199:0x0415, code lost:
        if (r0.zzD == -1.0f) goto L_0x04ca;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:201:0x041b, code lost:
        if (r0.zzE == -1.0f) goto L_0x04ca;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:203:0x0421, code lost:
        if (r0.zzF == -1.0f) goto L_0x04ca;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:205:0x0427, code lost:
        if (r0.zzG == -1.0f) goto L_0x04ca;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:207:0x042d, code lost:
        if (r0.zzH == -1.0f) goto L_0x04ca;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:209:0x0433, code lost:
        if (r0.zzI == -1.0f) goto L_0x04ca;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:211:0x0439, code lost:
        if (r0.zzJ == -1.0f) goto L_0x04ca;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:213:0x043f, code lost:
        if (r0.zzK == -1.0f) goto L_0x04ca;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:215:0x0445, code lost:
        if (r0.zzL == -1.0f) goto L_0x04ca;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:217:0x044b, code lost:
        if (r0.zzM != -1.0f) goto L_0x044f;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:218:0x044f, code lost:
        r6 = new byte[25];
        r8 = java.nio.ByteBuffer.wrap(r6).order(java.nio.ByteOrder.LITTLE_ENDIAN);
        r8.put((byte) 0);
        r8.putShort((short) ((int) ((r0.zzD * 50000.0f) + 0.5f)));
        r8.putShort((short) ((int) ((r0.zzE * 50000.0f) + 0.5f)));
        r8.putShort((short) ((int) ((r0.zzF * 50000.0f) + 0.5f)));
        r8.putShort((short) ((int) ((r0.zzG * 50000.0f) + 0.5f)));
        r8.putShort((short) ((int) ((r0.zzH * 50000.0f) + 0.5f)));
        r8.putShort((short) ((int) ((r0.zzI * 50000.0f) + 0.5f)));
        r8.putShort((short) ((int) ((r0.zzJ * 50000.0f) + 0.5f)));
        r8.putShort((short) ((int) ((r0.zzK * 50000.0f) + 0.5f)));
        r8.putShort((short) ((int) (r0.zzL + 0.5f)));
        r8.putShort((short) ((int) (r0.zzM + 0.5f)));
        r8.putShort((short) r0.zzB);
        r8.putShort((short) r0.zzC);
     */
    /* JADX WARNING: Code restructure failed: missing block: B:219:0x04ca, code lost:
        r6 = null;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:220:0x04cb, code lost:
        r8 = new com.google.android.gms.internal.ads.zzm();
        r8.zzc(r0.zzy);
        r8.zzb(r0.zzA);
        r8.zzd(r0.zzz);
        r8.zze(r6);
        r8.zzf(r0.zzn);
        r8.zza(r0.zzn);
        r6 = r8.zzg();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:221:0x04f1, code lost:
        r6 = null;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:223:0x04f4, code lost:
        if (r0.zza == null) goto L_0x0512;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:225:0x0500, code lost:
        if (com.google.android.gms.internal.ads.zzahj.zzf.containsKey(r0.zza) == false) goto L_0x0512;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:226:0x0502, code lost:
        r13 = ((java.lang.Integer) com.google.android.gms.internal.ads.zzahj.zzf.get(r0.zza)).intValue();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:228:0x0514, code lost:
        if (r0.zzr != 0) goto L_0x0562;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:230:0x051d, code lost:
        if (java.lang.Float.compare(r0.zzs, 0.0f) != 0) goto L_0x0562;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:232:0x0525, code lost:
        if (java.lang.Float.compare(r0.zzt, 0.0f) != 0) goto L_0x0562;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:234:0x052d, code lost:
        if (java.lang.Float.compare(r0.zzu, 0.0f) != 0) goto L_0x0530;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:236:0x0538, code lost:
        if (java.lang.Float.compare(r0.zzu, 90.0f) != 0) goto L_0x053d;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:237:0x053a, code lost:
        r11 = 90;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:239:0x0545, code lost:
        if (java.lang.Float.compare(r0.zzu, -180.0f) == 0) goto L_0x055f;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:241:0x054f, code lost:
        if (java.lang.Float.compare(r0.zzu, 180.0f) != 0) goto L_0x0552;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:243:0x055a, code lost:
        if (java.lang.Float.compare(r0.zzu, -90.0f) != 0) goto L_0x0562;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:244:0x055c, code lost:
        r11 = 270;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:245:0x055f, code lost:
        r11 = 180;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:246:0x0562, code lost:
        r11 = r13;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:247:0x0563, code lost:
        r7.zzac(r0.zzl);
        r7.zzI(r0.zzm);
        r7.zzT(r9);
        r7.zzW(r11);
        r7.zzU(r0.zzv);
        r7.zzaa(r0.zzw);
        r7.zzA(r6);
        r6 = 2;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:249:0x0586, code lost:
        if ("application/x-subrip".equals(r4) != false) goto L_0x05b7;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:251:0x058c, code lost:
        if ("text/x-ssa".equals(r4) != false) goto L_0x05b7;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:253:0x0594, code lost:
        if ("text/vtt".equals(r4) != false) goto L_0x05b7;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:255:0x059c, code lost:
        if ("application/vobsub".equals(r4) != false) goto L_0x05b7;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:257:0x05a4, code lost:
        if ("application/pgs".equals(r4) != false) goto L_0x05b7;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:259:0x05ac, code lost:
        if ("application/dvbsubs".equals(r4) == false) goto L_0x05af;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:261:0x05b6, code lost:
        throw com.google.android.gms.internal.ads.zzbo.zza("Unexpected MIME type.", (java.lang.Throwable) null);
     */
    /* JADX WARNING: Code restructure failed: missing block: B:262:0x05b7, code lost:
        r6 = 3;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:264:0x05ba, code lost:
        if (r0.zza == null) goto L_0x05cd;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:266:0x05c6, code lost:
        if (com.google.android.gms.internal.ads.zzahj.zzf.containsKey(r0.zza) != false) goto L_0x05cd;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:267:0x05c8, code lost:
        r7.zzM(r0.zza);
     */
    /* JADX WARNING: Code restructure failed: missing block: B:268:0x05cd, code lost:
        r7.zzJ(r21);
        r7.zzX(r4);
        r7.zzP(r1);
        r7.zzO(r0.zzZ);
        r7.zzZ(r5);
        r7.zzL(r2);
        r7.zzz(r3);
        r7.zzE(r0.zzk);
        r1 = r7.zzad();
        r2 = r20.zzw(r0.zzc, r6);
        r0.zzW = r2;
        r2.zzl(r1);
     */
    /* JADX WARNING: Code restructure failed: missing block: B:269:0x05fc, code lost:
        return;
     */
    /* JADX WARNING: Multi-variable type inference failed */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public final void zze(com.google.android.gms.internal.ads.zzacn r20, int r21) throws com.google.android.gms.internal.ads.zzbo {
        /*
            r19 = this;
            r0 = r19
            java.lang.String r1 = r0.zzb
            int r2 = r1.hashCode()
            r3 = 24
            r5 = 16
            r6 = 1
            r8 = 32
            r9 = 8
            r10 = 4
            r11 = 0
            r12 = 3
            r13 = -1
            switch(r2) {
                case -2095576542: goto L_0x0187;
                case -2095575984: goto L_0x017d;
                case -1985379776: goto L_0x0172;
                case -1784763192: goto L_0x0167;
                case -1730367663: goto L_0x015c;
                case -1482641358: goto L_0x0151;
                case -1482641357: goto L_0x0146;
                case -1373388978: goto L_0x013b;
                case -933872740: goto L_0x0131;
                case -538363189: goto L_0x0126;
                case -538363109: goto L_0x011b;
                case -425012669: goto L_0x010f;
                case -356037306: goto L_0x0103;
                case 62923557: goto L_0x00f7;
                case 62923603: goto L_0x00ec;
                case 62927045: goto L_0x00e0;
                case 82318131: goto L_0x00d5;
                case 82338133: goto L_0x00ca;
                case 82338134: goto L_0x00bf;
                case 99146302: goto L_0x00b3;
                case 444813526: goto L_0x00a7;
                case 542569478: goto L_0x009b;
                case 635596514: goto L_0x008f;
                case 725948237: goto L_0x0083;
                case 725957860: goto L_0x0078;
                case 738597099: goto L_0x006c;
                case 855502857: goto L_0x0061;
                case 1045209816: goto L_0x0055;
                case 1422270023: goto L_0x0049;
                case 1809237540: goto L_0x003e;
                case 1950749482: goto L_0x0032;
                case 1950789798: goto L_0x0026;
                case 1951062397: goto L_0x001a;
                default: goto L_0x0018;
            }
        L_0x0018:
            goto L_0x0191
        L_0x001a:
            java.lang.String r2 = "A_OPUS"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 12
            goto L_0x0192
        L_0x0026:
            java.lang.String r2 = "A_FLAC"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 22
            goto L_0x0192
        L_0x0032:
            java.lang.String r2 = "A_EAC3"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 17
            goto L_0x0192
        L_0x003e:
            java.lang.String r2 = "V_MPEG2"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = r12
            goto L_0x0192
        L_0x0049:
            java.lang.String r2 = "S_TEXT/UTF8"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 27
            goto L_0x0192
        L_0x0055:
            java.lang.String r2 = "S_TEXT/WEBVTT"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 29
            goto L_0x0192
        L_0x0061:
            java.lang.String r2 = "V_MPEGH/ISO/HEVC"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = r9
            goto L_0x0192
        L_0x006c:
            java.lang.String r2 = "S_TEXT/ASS"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 28
            goto L_0x0192
        L_0x0078:
            java.lang.String r2 = "A_PCM/INT/LIT"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = r3
            goto L_0x0192
        L_0x0083:
            java.lang.String r2 = "A_PCM/INT/BIG"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 25
            goto L_0x0192
        L_0x008f:
            java.lang.String r2 = "A_PCM/FLOAT/IEEE"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 26
            goto L_0x0192
        L_0x009b:
            java.lang.String r2 = "A_DTS/EXPRESS"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 20
            goto L_0x0192
        L_0x00a7:
            java.lang.String r2 = "V_THEORA"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 10
            goto L_0x0192
        L_0x00b3:
            java.lang.String r2 = "S_HDMV/PGS"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 31
            goto L_0x0192
        L_0x00bf:
            java.lang.String r2 = "V_VP9"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = r6
            goto L_0x0192
        L_0x00ca:
            java.lang.String r2 = "V_VP8"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = r11
            goto L_0x0192
        L_0x00d5:
            java.lang.String r2 = "V_AV1"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 2
            goto L_0x0192
        L_0x00e0:
            java.lang.String r2 = "A_DTS"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 19
            goto L_0x0192
        L_0x00ec:
            java.lang.String r2 = "A_AC3"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = r5
            goto L_0x0192
        L_0x00f7:
            java.lang.String r2 = "A_AAC"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 13
            goto L_0x0192
        L_0x0103:
            java.lang.String r2 = "A_DTS/LOSSLESS"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 21
            goto L_0x0192
        L_0x010f:
            java.lang.String r2 = "S_VOBSUB"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 30
            goto L_0x0192
        L_0x011b:
            java.lang.String r2 = "V_MPEG4/ISO/AVC"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 7
            goto L_0x0192
        L_0x0126:
            java.lang.String r2 = "V_MPEG4/ISO/ASP"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 5
            goto L_0x0192
        L_0x0131:
            java.lang.String r2 = "S_DVBSUB"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = r8
            goto L_0x0192
        L_0x013b:
            java.lang.String r2 = "V_MS/VFW/FOURCC"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 9
            goto L_0x0192
        L_0x0146:
            java.lang.String r2 = "A_MPEG/L3"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 15
            goto L_0x0192
        L_0x0151:
            java.lang.String r2 = "A_MPEG/L2"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 14
            goto L_0x0192
        L_0x015c:
            java.lang.String r2 = "A_VORBIS"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 11
            goto L_0x0192
        L_0x0167:
            java.lang.String r2 = "A_TRUEHD"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 18
            goto L_0x0192
        L_0x0172:
            java.lang.String r2 = "A_MS/ACM"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 23
            goto L_0x0192
        L_0x017d:
            java.lang.String r2 = "V_MPEG4/ISO/SP"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = r10
            goto L_0x0192
        L_0x0187:
            java.lang.String r2 = "V_MPEG4/ISO/AP"
            boolean r2 = r1.equals(r2)
            if (r2 == 0) goto L_0x0191
            r2 = 6
            goto L_0x0192
        L_0x0191:
            r2 = r13
        L_0x0192:
            java.lang.String r14 = "text/x-ssa"
            java.lang.String r15 = "application/x-subrip"
            r16 = 4096(0x1000, float:5.74E-42)
            java.lang.String r17 = "audio/raw"
            java.lang.String r18 = "audio/x-unknown"
            java.lang.String r7 = "MatroskaExtractor"
            java.lang.String r4 = ". Setting mimeType to audio/x-unknown"
            switch(r2) {
                case 0: goto L_0x039d;
                case 1: goto L_0x039a;
                case 2: goto L_0x0397;
                case 3: goto L_0x0394;
                case 4: goto L_0x0387;
                case 5: goto L_0x0387;
                case 6: goto L_0x0387;
                case 7: goto L_0x036a;
                case 8: goto L_0x0350;
                case 9: goto L_0x0333;
                case 10: goto L_0x032f;
                case 11: goto L_0x031d;
                case 12: goto L_0x02de;
                case 13: goto L_0x02be;
                case 14: goto L_0x02b7;
                case 15: goto L_0x02b4;
                case 16: goto L_0x02b0;
                case 17: goto L_0x02ac;
                case 18: goto L_0x02a1;
                case 19: goto L_0x029d;
                case 20: goto L_0x029d;
                case 21: goto L_0x0299;
                case 22: goto L_0x028d;
                case 23: goto L_0x0252;
                case 24: goto L_0x0230;
                case 25: goto L_0x0206;
                case 26: goto L_0x01eb;
                case 27: goto L_0x01e5;
                case 28: goto L_0x01d0;
                case 29: goto L_0x01cc;
                case 30: goto L_0x01c0;
                case 31: goto L_0x01bc;
                case 32: goto L_0x01ab;
                default: goto L_0x01a3;
            }
        L_0x01a3:
            java.lang.String r1 = "Unrecognized codec identifier."
            r2 = 0
            com.google.android.gms.internal.ads.zzbo r1 = com.google.android.gms.internal.ads.zzbo.zza(r1, r2)
            throw r1
        L_0x01ab:
            byte[] r2 = new byte[r10]
            byte[] r1 = r0.zzi(r1)
            java.lang.System.arraycopy(r1, r11, r2, r11, r10)
            com.google.android.gms.internal.ads.zzfxr r1 = com.google.android.gms.internal.ads.zzfxr.zzn(r2)
            java.lang.String r17 = "application/dvbsubs"
            goto L_0x034c
        L_0x01bc:
            java.lang.String r17 = "application/pgs"
            goto L_0x039f
        L_0x01c0:
            byte[] r1 = r0.zzi(r1)
            com.google.android.gms.internal.ads.zzfxr r1 = com.google.android.gms.internal.ads.zzfxr.zzn(r1)
            java.lang.String r17 = "application/vobsub"
            goto L_0x034c
        L_0x01cc:
            java.lang.String r17 = "text/vtt"
            goto L_0x039f
        L_0x01d0:
            byte[] r1 = com.google.android.gms.internal.ads.zzahj.zzb
            java.lang.String r2 = r0.zzb
            byte[] r2 = r0.zzi(r2)
            com.google.android.gms.internal.ads.zzfxr r1 = com.google.android.gms.internal.ads.zzfxr.zzo(r1, r2)
            r2 = r1
            r1 = r13
            r10 = r1
            r17 = r14
            goto L_0x03a2
        L_0x01e5:
            r1 = r13
            r10 = r1
            r17 = r15
            goto L_0x03a1
        L_0x01eb:
            int r1 = r0.zzP
            if (r1 != r8) goto L_0x01f0
            goto L_0x024f
        L_0x01f0:
            java.lang.StringBuilder r2 = new java.lang.StringBuilder
            java.lang.String r3 = "Unsupported floating point PCM bit depth: "
            r2.<init>(r3)
            r2.append(r1)
            r2.append(r4)
            java.lang.String r1 = r2.toString()
            com.google.android.gms.internal.ads.zzea.zzf(r7, r1)
            goto L_0x0287
        L_0x0206:
            int r1 = r0.zzP
            if (r1 != r9) goto L_0x020c
            r10 = r12
            goto L_0x024f
        L_0x020c:
            if (r1 != r5) goto L_0x0211
            r10 = 268435456(0x10000000, float:2.5243549E-29)
            goto L_0x024f
        L_0x0211:
            if (r1 != r3) goto L_0x0216
            r10 = 1342177280(0x50000000, float:8.5899346E9)
            goto L_0x024f
        L_0x0216:
            if (r1 != r8) goto L_0x021b
            r10 = 1610612736(0x60000000, float:3.6893488E19)
            goto L_0x024f
        L_0x021b:
            java.lang.StringBuilder r2 = new java.lang.StringBuilder
            java.lang.String r3 = "Unsupported big endian PCM bit depth: "
            r2.<init>(r3)
            r2.append(r1)
            r2.append(r4)
            java.lang.String r1 = r2.toString()
            com.google.android.gms.internal.ads.zzea.zzf(r7, r1)
            goto L_0x0287
        L_0x0230:
            int r1 = r0.zzP
            int r10 = com.google.android.gms.internal.ads.zzet.zzl(r1)
            if (r10 != 0) goto L_0x024f
            int r1 = r0.zzP
            java.lang.StringBuilder r2 = new java.lang.StringBuilder
            java.lang.String r3 = "Unsupported little endian PCM bit depth: "
            r2.<init>(r3)
            r2.append(r1)
            r2.append(r4)
            java.lang.String r1 = r2.toString()
            com.google.android.gms.internal.ads.zzea.zzf(r7, r1)
            goto L_0x0287
        L_0x024f:
            r1 = r13
            goto L_0x03a1
        L_0x0252:
            com.google.android.gms.internal.ads.zzek r1 = new com.google.android.gms.internal.ads.zzek
            java.lang.String r2 = r0.zzb
            byte[] r2 = r0.zzi(r2)
            r1.<init>((byte[]) r2)
            boolean r1 = zzh(r1)
            if (r1 == 0) goto L_0x0282
            int r1 = r0.zzP
            int r10 = com.google.android.gms.internal.ads.zzet.zzl(r1)
            if (r10 != 0) goto L_0x024f
            int r1 = r0.zzP
            java.lang.StringBuilder r2 = new java.lang.StringBuilder
            java.lang.String r3 = "Unsupported PCM bit depth: "
            r2.<init>(r3)
            r2.append(r1)
            r2.append(r4)
            java.lang.String r1 = r2.toString()
            com.google.android.gms.internal.ads.zzea.zzf(r7, r1)
            goto L_0x0287
        L_0x0282:
            java.lang.String r1 = "Non-PCM MS/ACM is unsupported. Setting mimeType to audio/x-unknown"
            com.google.android.gms.internal.ads.zzea.zzf(r7, r1)
        L_0x0287:
            r1 = r13
            r10 = r1
            r17 = r18
            goto L_0x03a1
        L_0x028d:
            byte[] r1 = r0.zzi(r1)
            java.util.List r1 = java.util.Collections.singletonList(r1)
            java.lang.String r17 = "audio/flac"
            goto L_0x034c
        L_0x0299:
            java.lang.String r17 = "audio/vnd.dts.hd"
            goto L_0x039f
        L_0x029d:
            java.lang.String r17 = "audio/vnd.dts"
            goto L_0x039f
        L_0x02a1:
            com.google.android.gms.internal.ads.zzadq r1 = new com.google.android.gms.internal.ads.zzadq
            r1.<init>()
            r0.zzT = r1
            java.lang.String r17 = "audio/true-hd"
            goto L_0x039f
        L_0x02ac:
            java.lang.String r17 = "audio/eac3"
            goto L_0x039f
        L_0x02b0:
            java.lang.String r17 = "audio/ac3"
            goto L_0x039f
        L_0x02b4:
            java.lang.String r17 = "audio/mpeg"
            goto L_0x02b9
        L_0x02b7:
            java.lang.String r17 = "audio/mpeg-L2"
        L_0x02b9:
            r10 = r13
            r1 = r16
            goto L_0x03a1
        L_0x02be:
            byte[] r1 = r0.zzi(r1)
            java.util.List r1 = java.util.Collections.singletonList(r1)
            byte[] r2 = r0.zzj
            com.google.android.gms.internal.ads.zzabf r2 = com.google.android.gms.internal.ads.zzabg.zza(r2)
            int r3 = r2.zza
            r0.zzQ = r3
            int r3 = r2.zzb
            r0.zzO = r3
            java.lang.String r2 = r2.zzc
            java.lang.String r17 = "audio/mp4a-latm"
            r3 = r2
            r10 = r13
            r2 = r1
            r1 = r10
            goto L_0x03a3
        L_0x02de:
            java.util.ArrayList r1 = new java.util.ArrayList
            r1.<init>(r12)
            java.lang.String r2 = r0.zzb
            byte[] r2 = r0.zzi(r2)
            r1.add(r2)
            java.nio.ByteBuffer r2 = java.nio.ByteBuffer.allocate(r9)
            java.nio.ByteOrder r3 = java.nio.ByteOrder.LITTLE_ENDIAN
            java.nio.ByteBuffer r2 = r2.order(r3)
            long r4 = r0.zzR
            java.nio.ByteBuffer r2 = r2.putLong(r4)
            byte[] r2 = r2.array()
            r1.add(r2)
            java.nio.ByteBuffer r2 = java.nio.ByteBuffer.allocate(r9)
            java.nio.ByteBuffer r2 = r2.order(r3)
            long r3 = r0.zzS
            java.nio.ByteBuffer r2 = r2.putLong(r3)
            byte[] r2 = r2.array()
            r1.add(r2)
            r16 = 5760(0x1680, float:8.071E-42)
            java.lang.String r17 = "audio/opus"
            goto L_0x0329
        L_0x031d:
            byte[] r1 = r0.zzi(r1)
            java.util.List r1 = zzg(r1)
            r16 = 8192(0x2000, float:1.14794E-41)
            java.lang.String r17 = "audio/vorbis"
        L_0x0329:
            r2 = r1
            r10 = r13
            r1 = r16
            goto L_0x03a2
        L_0x032f:
            java.lang.String r17 = "video/x-unknown"
            goto L_0x039f
        L_0x0333:
            com.google.android.gms.internal.ads.zzek r1 = new com.google.android.gms.internal.ads.zzek
            java.lang.String r2 = r0.zzb
            byte[] r2 = r0.zzi(r2)
            r1.<init>((byte[]) r2)
            android.util.Pair r1 = zzf(r1)
            java.lang.Object r2 = r1.first
            r17 = r2
            java.lang.String r17 = (java.lang.String) r17
            java.lang.Object r1 = r1.second
            java.util.List r1 = (java.util.List) r1
        L_0x034c:
            r2 = r1
            r1 = r13
            r10 = r1
            goto L_0x03a2
        L_0x0350:
            com.google.android.gms.internal.ads.zzek r1 = new com.google.android.gms.internal.ads.zzek
            java.lang.String r2 = r0.zzb
            byte[] r2 = r0.zzi(r2)
            r1.<init>((byte[]) r2)
            com.google.android.gms.internal.ads.zzacz r1 = com.google.android.gms.internal.ads.zzacz.zza(r1)
            java.util.List r2 = r1.zza
            int r3 = r1.zzb
            r0.zzX = r3
            java.lang.String r1 = r1.zzi
            java.lang.String r17 = "video/hevc"
            goto L_0x0383
        L_0x036a:
            com.google.android.gms.internal.ads.zzek r1 = new com.google.android.gms.internal.ads.zzek
            java.lang.String r2 = r0.zzb
            byte[] r2 = r0.zzi(r2)
            r1.<init>((byte[]) r2)
            com.google.android.gms.internal.ads.zzabn r1 = com.google.android.gms.internal.ads.zzabn.zza(r1)
            java.util.List r2 = r1.zza
            int r3 = r1.zzb
            r0.zzX = r3
            java.lang.String r1 = r1.zzk
            java.lang.String r17 = "video/avc"
        L_0x0383:
            r3 = r1
            r1 = r13
            r10 = r1
            goto L_0x03a3
        L_0x0387:
            byte[] r1 = r0.zzj
            if (r1 != 0) goto L_0x038d
            r1 = 0
            goto L_0x0391
        L_0x038d:
            java.util.List r1 = java.util.Collections.singletonList(r1)
        L_0x0391:
            java.lang.String r17 = "video/mp4v-es"
            goto L_0x034c
        L_0x0394:
            java.lang.String r17 = "video/mpeg2"
            goto L_0x039f
        L_0x0397:
            java.lang.String r17 = "video/av01"
            goto L_0x039f
        L_0x039a:
            java.lang.String r17 = "video/x-vnd.on2.vp9"
            goto L_0x039f
        L_0x039d:
            java.lang.String r17 = "video/x-vnd.on2.vp8"
        L_0x039f:
            r1 = r13
            r10 = r1
        L_0x03a1:
            r2 = 0
        L_0x03a2:
            r3 = 0
        L_0x03a3:
            byte[] r4 = r0.zzN
            if (r4 == 0) goto L_0x03b8
            com.google.android.gms.internal.ads.zzek r4 = new com.google.android.gms.internal.ads.zzek
            byte[] r5 = r0.zzN
            r4.<init>((byte[]) r5)
            com.google.android.gms.internal.ads.zzace r4 = com.google.android.gms.internal.ads.zzace.zza(r4)
            if (r4 == 0) goto L_0x03b8
            java.lang.String r3 = r4.zza
            java.lang.String r17 = "video/dolby-vision"
        L_0x03b8:
            r4 = r17
            boolean r5 = r0.zzV
            boolean r7 = r0.zzU
            if (r6 == r7) goto L_0x03c2
            r7 = r11
            goto L_0x03c3
        L_0x03c2:
            r7 = 2
        L_0x03c3:
            r5 = r5 | r7
            com.google.android.gms.internal.ads.zzad r7 = new com.google.android.gms.internal.ads.zzad
            r7.<init>()
            boolean r8 = com.google.android.gms.internal.ads.zzbn.zzg(r4)
            if (r8 == 0) goto L_0x03de
            int r8 = r0.zzO
            r7.zzy(r8)
            int r8 = r0.zzQ
            r7.zzY(r8)
            r7.zzR(r10)
            goto L_0x05b8
        L_0x03de:
            boolean r6 = com.google.android.gms.internal.ads.zzbn.zzi(r4)
            if (r6 == 0) goto L_0x0582
            int r6 = r0.zzq
            if (r6 != 0) goto L_0x03f8
            int r6 = r0.zzo
            if (r6 != r13) goto L_0x03ee
            int r6 = r0.zzl
        L_0x03ee:
            r0.zzo = r6
            int r6 = r0.zzp
            if (r6 != r13) goto L_0x03f6
            int r6 = r0.zzm
        L_0x03f6:
            r0.zzp = r6
        L_0x03f8:
            int r6 = r0.zzo
            r8 = -1082130432(0xffffffffbf800000, float:-1.0)
            if (r6 == r13) goto L_0x040c
            int r9 = r0.zzp
            if (r9 == r13) goto L_0x040c
            int r10 = r0.zzm
            int r10 = r10 * r6
            int r6 = r0.zzl
            int r6 = r6 * r9
            float r9 = (float) r10
            float r6 = (float) r6
            float r9 = r9 / r6
            goto L_0x040d
        L_0x040c:
            r9 = r8
        L_0x040d:
            boolean r6 = r0.zzx
            if (r6 == 0) goto L_0x04f1
            float r6 = r0.zzD
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 == 0) goto L_0x04ca
            float r6 = r0.zzE
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 == 0) goto L_0x04ca
            float r6 = r0.zzF
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 == 0) goto L_0x04ca
            float r6 = r0.zzG
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 == 0) goto L_0x04ca
            float r6 = r0.zzH
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 == 0) goto L_0x04ca
            float r6 = r0.zzI
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 == 0) goto L_0x04ca
            float r6 = r0.zzJ
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 == 0) goto L_0x04ca
            float r6 = r0.zzK
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 == 0) goto L_0x04ca
            float r6 = r0.zzL
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 == 0) goto L_0x04ca
            float r6 = r0.zzM
            int r6 = (r6 > r8 ? 1 : (r6 == r8 ? 0 : -1))
            if (r6 != 0) goto L_0x044f
            goto L_0x04ca
        L_0x044f:
            r6 = 25
            byte[] r6 = new byte[r6]
            java.nio.ByteBuffer r8 = java.nio.ByteBuffer.wrap(r6)
            java.nio.ByteOrder r10 = java.nio.ByteOrder.LITTLE_ENDIAN
            java.nio.ByteBuffer r8 = r8.order(r10)
            r8.put(r11)
            float r10 = r0.zzD
            r12 = 1195593728(0x47435000, float:50000.0)
            float r10 = r10 * r12
            r14 = 1056964608(0x3f000000, float:0.5)
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            float r10 = r0.zzE
            float r10 = r10 * r12
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            float r10 = r0.zzF
            float r10 = r10 * r12
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            float r10 = r0.zzG
            float r10 = r10 * r12
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            float r10 = r0.zzH
            float r10 = r10 * r12
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            float r10 = r0.zzI
            float r10 = r10 * r12
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            float r10 = r0.zzJ
            float r10 = r10 * r12
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            float r10 = r0.zzK
            float r10 = r10 * r12
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            float r10 = r0.zzL
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            float r10 = r0.zzM
            float r10 = r10 + r14
            int r10 = (int) r10
            short r10 = (short) r10
            r8.putShort(r10)
            int r10 = r0.zzB
            short r10 = (short) r10
            r8.putShort(r10)
            int r10 = r0.zzC
            short r10 = (short) r10
            r8.putShort(r10)
            goto L_0x04cb
        L_0x04ca:
            r6 = 0
        L_0x04cb:
            com.google.android.gms.internal.ads.zzm r8 = new com.google.android.gms.internal.ads.zzm
            r8.<init>()
            int r10 = r0.zzy
            r8.zzc(r10)
            int r10 = r0.zzA
            r8.zzb(r10)
            int r10 = r0.zzz
            r8.zzd(r10)
            r8.zze(r6)
            int r6 = r0.zzn
            r8.zzf(r6)
            int r6 = r0.zzn
            r8.zza(r6)
            com.google.android.gms.internal.ads.zzo r6 = r8.zzg()
            goto L_0x04f2
        L_0x04f1:
            r6 = 0
        L_0x04f2:
            java.lang.String r8 = r0.zza
            if (r8 == 0) goto L_0x0512
            java.util.Map r8 = com.google.android.gms.internal.ads.zzahj.zzf
            java.lang.String r10 = r0.zza
            boolean r8 = r8.containsKey(r10)
            if (r8 == 0) goto L_0x0512
            java.util.Map r8 = com.google.android.gms.internal.ads.zzahj.zzf
            java.lang.String r10 = r0.zza
            java.lang.Object r8 = r8.get(r10)
            java.lang.Integer r8 = (java.lang.Integer) r8
            int r13 = r8.intValue()
        L_0x0512:
            int r8 = r0.zzr
            if (r8 != 0) goto L_0x0562
            float r8 = r0.zzs
            r10 = 0
            int r8 = java.lang.Float.compare(r8, r10)
            if (r8 != 0) goto L_0x0562
            float r8 = r0.zzt
            int r8 = java.lang.Float.compare(r8, r10)
            if (r8 != 0) goto L_0x0562
            float r8 = r0.zzu
            int r8 = java.lang.Float.compare(r8, r10)
            if (r8 != 0) goto L_0x0530
            goto L_0x0563
        L_0x0530:
            float r8 = r0.zzu
            r10 = 1119092736(0x42b40000, float:90.0)
            int r8 = java.lang.Float.compare(r8, r10)
            if (r8 != 0) goto L_0x053d
            r11 = 90
            goto L_0x0563
        L_0x053d:
            float r8 = r0.zzu
            r10 = -1020002304(0xffffffffc3340000, float:-180.0)
            int r8 = java.lang.Float.compare(r8, r10)
            if (r8 == 0) goto L_0x055f
            float r8 = r0.zzu
            r10 = 1127481344(0x43340000, float:180.0)
            int r8 = java.lang.Float.compare(r8, r10)
            if (r8 != 0) goto L_0x0552
            goto L_0x055f
        L_0x0552:
            float r8 = r0.zzu
            r10 = -1028390912(0xffffffffc2b40000, float:-90.0)
            int r8 = java.lang.Float.compare(r8, r10)
            if (r8 != 0) goto L_0x0562
            r11 = 270(0x10e, float:3.78E-43)
            goto L_0x0563
        L_0x055f:
            r11 = 180(0xb4, float:2.52E-43)
            goto L_0x0563
        L_0x0562:
            r11 = r13
        L_0x0563:
            int r8 = r0.zzl
            r7.zzac(r8)
            int r8 = r0.zzm
            r7.zzI(r8)
            r7.zzT(r9)
            r7.zzW(r11)
            byte[] r8 = r0.zzv
            r7.zzU(r8)
            int r8 = r0.zzw
            r7.zzaa(r8)
            r7.zzA(r6)
            r6 = 2
            goto L_0x05b8
        L_0x0582:
            boolean r6 = r15.equals(r4)
            if (r6 != 0) goto L_0x05b7
            boolean r6 = r14.equals(r4)
            if (r6 != 0) goto L_0x05b7
            java.lang.String r6 = "text/vtt"
            boolean r6 = r6.equals(r4)
            if (r6 != 0) goto L_0x05b7
            java.lang.String r6 = "application/vobsub"
            boolean r6 = r6.equals(r4)
            if (r6 != 0) goto L_0x05b7
            java.lang.String r6 = "application/pgs"
            boolean r6 = r6.equals(r4)
            if (r6 != 0) goto L_0x05b7
            java.lang.String r6 = "application/dvbsubs"
            boolean r6 = r6.equals(r4)
            if (r6 == 0) goto L_0x05af
            goto L_0x05b7
        L_0x05af:
            java.lang.String r1 = "Unexpected MIME type."
            r2 = 0
            com.google.android.gms.internal.ads.zzbo r1 = com.google.android.gms.internal.ads.zzbo.zza(r1, r2)
            throw r1
        L_0x05b7:
            r6 = r12
        L_0x05b8:
            java.lang.String r8 = r0.zza
            if (r8 == 0) goto L_0x05cd
            java.util.Map r8 = com.google.android.gms.internal.ads.zzahj.zzf
            java.lang.String r9 = r0.zza
            boolean r8 = r8.containsKey(r9)
            if (r8 != 0) goto L_0x05cd
            java.lang.String r8 = r0.zza
            r7.zzM(r8)
        L_0x05cd:
            r8 = r21
            r7.zzJ(r8)
            r7.zzX(r4)
            r7.zzP(r1)
            java.lang.String r1 = r0.zzZ
            r7.zzO(r1)
            r7.zzZ(r5)
            r7.zzL(r2)
            r7.zzz(r3)
            com.google.android.gms.internal.ads.zzy r1 = r0.zzk
            r7.zzE(r1)
            com.google.android.gms.internal.ads.zzaf r1 = r7.zzad()
            int r2 = r0.zzc
            r3 = r20
            com.google.android.gms.internal.ads.zzadp r2 = r3.zzw(r2, r6)
            r0.zzW = r2
            r2.zzl(r1)
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.zzahi.zze(com.google.android.gms.internal.ads.zzacn, int):void");
    }
}
