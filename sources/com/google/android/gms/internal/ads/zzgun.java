...
public final class zzgun implements zzgdo {
    ...
    public static zzgdo zzb(zzgft zzgft) throws GeneralSecurityException {
        if (zzgkr.zza(1)) {
            zzgft.zzb();
            return new zzgun(zzgft.zzd().zzd(zzgdx.zza()), zzgft.zzb().zzb(), zzgft.zzc().zzc());
        }
        throw new GeneralSecurityException("Can not use AES-EAX in FIPS-mode.");
    }
    ...
    private static byte[] zzc(byte[] bArr) {
        ...
        while (i < 15) {
            byte b = bArr[i];
            int i2 = i + 1;
            bArr2[i] = (byte) (((b + b) ^ ((bArr[i2] & UnsignedBytes.MAX_VALUE) >>> 7)) & Constants.MAX_HOST_LENGTH);
            i = i2;
        }
        byte b2 = bArr[15];
